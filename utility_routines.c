...
FILE	*logoutput = NULL;	/* Default descriptor for default output device		*/
...
OPTS	*optp;
	...
	for (optp = opts; $ASCLEN(&optp->name); optp++)
		...
		switch ( optp->type )
			...
			case	OPTS$K_INT:
				$LOG(STS$K_INFO, "%.*s = %d (0x%X)", $ASC(&optp->name), *((int *) optp->ptr), *((int *) optp->ptr));
				break;
...
OPTS	*optp;
	...
	if ( fconf && (*fconf) )
		...
		if ( !(finp = fopen(fconf, "r")) )
			{
			$LOG(STS$K_FATAL, "Error open file '%s', errno = %d", fconf, errno);
			return	STS$K_SUCCESS;
			}
		}
	...
	*/
	for ( i = 1; i < argc; i++ )
		...
		if ( (*(argp = argv[i]) != '-') && (*(argp = argv[i]) != '/') )
			{
			$LOG(STS$K_ERROR, "%d: Option must be started with a '-', skip : '%s'", i, argv[i]);
			continue;
			}
...
struct timespec now;
	...
	*/
	for (i = 0; i < ((srclen / 16));  i++)
		...
		fwrite (out, sizeof(out) , 1, logoutput ? logoutput : stdout);
		}
...
struct timespec now;
	...
	** Some sanity check
	*/
	if ( !($ISINRANGE(sev, STS$K_WARN, STS$K_FATAL)) )
		sev = STS$K_UNDEF;
 ...
 *		File will be opene in shared for read and append mode!
 *
 *  Input:
...
time_t	now = 0;
	...
	/*
	 * Performs file buffers flushing at 15 seconds interval
	 */
	if ( (lastflush + 15) < time(&now) )
		{
		fflush (logoutput ? logoutput : stdout);
		lastflush = now;
		}
	...
	if ( 0 > (status = ftell (logoutput)) )
		return	$LOG(STS$K_ERROR, "ftell() -> %d", errno);

	if ( status < limit )
		return	STS$K_SUCCESS;
#ifndef	WIN32
	if ( ftruncate(fileno(logoutput), status ) == -1 )
		$LOG(STS$K_ERROR, "ftruncate() -> %d", errno);
#endif


	if ( 0 > fseek(logoutput, 0L, SEEK_SET) )
